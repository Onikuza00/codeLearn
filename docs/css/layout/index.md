# Layout CSS { .section-layout }

> Layout es **cómo distribuís los elementos** en una página. Flexbox para una dimensión, Grid para dos. Son las dos herramientas modernas que reemplazan floats, tables y position absoluto.

---

## ¿Qué problema resuelven?

Antes de Flexbox y Grid, hacer un layout de 3 columnas iguales requería floats con clearfix o tablas HTML. Era frágil, complicado y nada responsivo.

Hoy tenés dos sistemas:

| Sistema | Para qué | Analogía |
|---------|----------|----------|
| **Flexbox** | Distribuir items en UNA línea (fila o columna) | Una barra de herramientas |
| **Grid** | Distribuir items en una CUADRÍCULA (filas y columnas) | Un tablero de ajedrez |

No compiten. **Flexbox para componentes, Grid para páginas.**

---

## Bloques

| Bloque | Concepto | Estado |
|--------|----------|--------|
| **Flexbox** | Layout unidimensional (filas o columnas) | ✅ Completo |
| **Grid** | Layout bidimensional (filas Y columnas) | ✅ Completo |

---

## 📖 Referencias

- 📘 **MDN — Flexbox** — https://developer.mozilla.org/en-US/docs/Learn/CSS/CSS_layout/Flexbox
- 📘 **MDN — Grid** — https://developer.mozilla.org/en-US/docs/Learn/CSS/CSS_layout/Grid
- 🎥 **midudev — Flexbox** — https://www.youtube.com/watch?v=Vqd96mD4hjA
- 🎥 **midudev — Grid** — https://www.youtube.com/watch?v=HBm1oFJwVvs
- 🎮 **Flexbox Froggy** — https://flexboxfroggy.com/
- 🎮 **Grid Garden** — https://cssgridgarden.com/
