...
> Primera llamada real a la API: instalar el SDK, crear el cliente, y mandar la primera solicitud con `client.messages.create()`.
...
`client.messages.create()` devuelve una `Promise` — hay que usar `await` (o `.then()`) para obtener la respuesta.
